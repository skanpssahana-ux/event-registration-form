...
if student_options and event_options:
  ...
  if st.button("Confirm & Process Registration", type="primary"):
    with sqlite3.connect(DB_NAME) as conn:
      ...
      if cursor.fetchone():
        st.warning(
            f"⚠️ User Notification: {student_name} is already registered for"
            f" '{target_name}'!"
        )
      else:
        cursor.execute(
        ...
        if has_time_clash:
          st.error(
              f"❌ Schedule Conflict! Cannot register for '{target_name}'. It"
              f" conflicts with '{clashing_event_name}' scheduled at the exact"
              f" same time ({target_time})."
          )
        else:
